...
class TS_decay(uniform_decay):
    ...
    def __init__(self, no_arms: int, no_output: int = 1, 
        ...
        super(TS_decay, self).__init__(no_arms, no_output, conf_level, precision_level, stop_type, baseline, reward_type, time_decay)
    ...
    def update(self, arms: list, rewards: list)-> None:
        ...
        super(TS_decay, self).update(arms, rewards)

        # update process that is particular to Thompson Sampling
        for i, idx in enumerate(arms):
            self.prior_para_1[idx], self.prior_para_2[idx] = update_posterior(self.prior_para_1[idx], self.prior_para_2[idx], rewards[i], self.reward_type, self.time_decay)
