    ...
    def detect_change(self, arm, reward):
        ...
        if arm['t']<=self.time_window:
            ...
            if mod(arm['t'], self.time_window)==0:  
                thred = cb.upper(arm['mu_front'], self.conf_level/2, arm['break_point'], sigma = arm['mu_front_std']) + cb.upper(arm['mu_back'], self.conf_level/2, arm['t'] - arm['break_point'], sigma = arm['mu_back_std']) - arm['mu_front'] - arm['mu_back']
                diff = abs(arm['mu_back'] - arm['mu_front'])
                if diff > thred:
                    self.start(restart = True)
                    self.restart_time.append(self.t)
                    if self.verbose:
                        print(f'{Fore.RED}Restart time step: %d: diff: %.4f, thred: %.4f {Style.RESET_ALL} '%(self.t, diff, thred))
